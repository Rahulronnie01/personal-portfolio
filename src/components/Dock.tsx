...
const DOCK_ITEMS = [
  ...
  { id: "resume", icon: Icons.FileText, label: "Resume", href: "https://drive.google.com/uc?export=download&id=1Yqgu2QLdVzXBRmOvXy1rkuH3wEgEgSdP", external: true },
];
...
export default function Dock() {
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50 flex h-16 items-end gap-3 md:gap-4 rounded-2xl border border-white/10 bg-white/5 pb-3 px-3 md:px-4 backdrop-blur-md w-fit max-w-[90vw] overflow-x-auto md:overflow-visible [&::-webkit-scrollbar]:hidden"
    >
      {DOCK_ITEMS.map((item) => (
...
function DockIcon({
  ...
  return (
    <Link href={item.href} target={item.external ? "_blank" : undefined} className="shrink-0">
      <motion.div
        ref={ref}
        style={{ width }}
        className="max-md:!w-10 aspect-square shrink-0 w-10 flex items-center justify-center rounded-full bg-gray-700/50 border border-white/10 md:hover:bg-gray-600/80 active:bg-gray-600/80 transition-colors group relative"
      >
        <item.icon className="w-1/2 h-1/2 text-gray-200 group-hover:text-white" />

        {/* Tooltip */}
        <span className="absolute -top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-white/10 hidden md:block">
          {item.label}
        </span>
      </motion.div>
    </Link>
